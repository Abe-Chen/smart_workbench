...
class WeatherCardView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final bool hasMetrics = card.metrics.isNotEmpty;
    final bool hasTimeline = card.timeline.isNotEmpty;
    final Widget? body = (hasMetrics || hasTimeline)
        ? _WeatherBody(card: card)
        : null;
    ...
    return BaseAssistantCard(
      ...
      decoration: const BoxDecoration(
        ...
        border: Border.fromBorderSide(BorderSide(color: Color(0xFFD4E1FF))),
      ),
      hero: _WeatherHero(card: card, compact: compact),
      body: body,
      footer: _WeatherFooter(card: card, compact: compact),
    );
  }
}

class _WeatherHero extends StatelessWidget {
  const _WeatherHero({required this.card, required this.compact});
      ...
      children.add(
        Wrap(
          ...
          children: card.metrics
              .map(
                (AssistantResultMetric metric) => _MetricChip(metric: metric),
              )
              .toList(),
        ),
      );
...
class _WeatherFooter extends StatelessWidget {
  const _WeatherFooter({required this.card, required this.compact});
